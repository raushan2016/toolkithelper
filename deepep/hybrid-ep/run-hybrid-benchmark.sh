...
echo "Deploying Configuration: ${NODES} Nodes, ${GPUS_PER_NODE} GPUs/Node, Test Type: ${TEST_TYPE} in Namespace: '${NAMESPACE}'"

echo "Cleaning up any existing benchmark run..."
kubectl delete statefulset hybrid-ep-benchmark -n "${NAMESPACE}" --ignore-not-found=true
kubectl delete deployment etcd -n "${NAMESPACE}" --ignore-not-found=true
kubectl delete service etcd -n "${NAMESPACE}" --ignore-not-found=true
kubectl delete service hybrid-ep-headless -n "${NAMESPACE}" --ignore-not-found=true
...
envsubst '$NODES,$NAMESPACE,$GPUS_PER_NODE,$IMAGE,$TEST_TYPE' < hybrid-ep-benchmark.yaml | kubectl apply -f -

echo ""
echo "Deployment applied! Waiting for StatefulSet to rollout..."
kubectl rollout status statefulset hybrid-ep-benchmark -n "${NAMESPACE}"
...
echo "Tailing logs from hybrid-ep-benchmark-0..."
# Stream logs in background so we can reliably kill it when finished
kubectl logs -f hybrid-ep-benchmark-0 -n "${NAMESPACE}" &
LOG_PID=$!

# Monitor for completion
while true; do
  if kubectl logs hybrid-ep-benchmark-0 -n "${NAMESPACE}" --tail=50 2>/dev/null | grep -q "Benchmark Finished"; then
...
DMA_BUF_COUNT=$(kubectl logs hybrid-ep-benchmark-0 -n "${NAMESPACE}" | grep -c "ibv_reg_dmabuf_mr" || true)
...
kubectl logs hybrid-ep-benchmark-0 -n "${NAMESPACE}" | grep "\[tuning\] Best" | while read -r line; do
...
echo "    $ kubectl logs hybrid-ep-benchmark-0 -n ${NAMESPACE}"
echo "======================================================================"
